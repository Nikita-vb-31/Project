...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	/* -- DIRECTW -- */
	function Shaded_box($text, $font = '', $fontstyle = 'B', $szfont = '', $width = '70%', $style = 'DF', $radius = 2.5, $fill = '#FFFFFF', $color = '#000', $pad = 2)
	{
	...
	function _createHTMLheaderFooter($arr, $hf)
	{
		...
		$html = '<table width="100%" style="border-collapse: collapse; margin: 0; vertical-align: ' . $valign . '; color: #000; ';
		if (isset($arr['line']) && $arr['line']) {
			$html .= ' border-' . $valign . ': 0.1mm solid #000;';
		}
		$html .= '">';
		$html .= '<tr>';
	...
	function SetHeader($Harray = [], $side = '', $write = false)
	{
		...
		if (is_string($Harray)) {
			...
			if (strlen($Harray) === 0) {
				...
				$oddhtml = '<table width="100%" style="border-collapse: collapse; margin: 0; vertical-align: bottom; color: #000; ';
				...
				if ($this->defaultheaderline) {
					$oddhtml .= ' border-bottom: 0.1mm solid #000;';
				}
				...
				$evenhtml = '<table width="100%" style="border-collapse: collapse; margin: 0; vertical-align: bottom; color: #000; ';
				...
				if ($this->defaultheaderline) {
					$evenhtml .= ' border-bottom: 0.1mm solid #000;';
				}
				...
				$oddhtml = '<div style="margin: 0; color: #000; ';
				...
				$evenhtml = '<div style="margin: 0; color: #000; ';
	...
	function SetFooter($Farray = [], $side = '')
	{
		...
		if (is_string($Farray)) {
			...
			if (strlen($Farray) == 0) {
				...
				$hdet = explode('|', $Farray);
				$oddhtml = '<table width="100%" style="border-collapse: collapse; margin: 0; vertical-align: top; color: #000; ';
				...
				if ($this->defaultfooterline) {
					$oddhtml .= ' border-top: 0.1mm solid #000;';
				}
				...
				$evenhtml = '<table width="100%" style="border-collapse: collapse; margin: 0; vertical-align: top; color: #000; ';
				...
				if ($this->defaultfooterline) {
					$evenhtml .= ' border-top: 0.1mm solid #000;';
				}
	...
	function border_details($bd)
	{
		...
		if (count($prop) == 1) {
			...
			// Change #000 1px solid to 1px solid #000 (proper)
			if (substr($prop[0], 0, 1) == '#') {
				$tmp = $prop[0];
				$prop[0] = $prop[1];
				$prop[1] = $prop[2];
				$prop[2] = $tmp;
			} // Change solid #000 1px to 1px solid #000 (proper)
			elseif (substr($prop[0], 1, 1) == '#') {
				$tmp = $prop[1];
				$prop[0] = $prop[2];
				$prop[1] = $prop[0];
				$prop[2] = $tmp;
			} // Change solid 1px #000 to 1px solid #000 (proper)
			elseif (in_array($prop[0], $this->borderstyles) || $prop[0] == 'none' || $prop[0] == 'hidden') {
				$tmp = $prop[0];
				$prop[0] = $prop[1];
				$prop[1] = $tmp;
